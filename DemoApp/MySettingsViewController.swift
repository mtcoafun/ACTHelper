...
class MySettingsViewController: BaseViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc func accountSetting(_ sender: UIButton)
    {
        if DataUtil.isVisitorLogin()
        {
            self.alertWhenYouKe()
    ...
    func alertWhenYouKe()
    {
        let alert = UIAlertController(title: "提示", message: "该功能仅限登陆会员使用，\n您需要重新登陆吗?", preferredStyle: UIAlertController.Style.alert)
        
        let yes = UIAlertAction(title: "是", style: UIAlertAction.Style.default, handler: {(alerts: UIAlertAction) -> Void in
            
            self.goBackToLogin()
        })
        
        let no = UIAlertAction(title: "否", style: UIAlertAction.Style.default, handler: {(alerts: UIAlertAction) -> Void in
    ...
    @objc func feedback(_ sender: UIButton)
    {
        let vc = FeedBackViewController()
        self.navigationController?.pushViewController(vc, animated: true)
    }
    
    @objc func pingFen(_ sender: UIButton)
    {
    ...
    @objc func share(_ sender: UIButton)
    {
        ...
        shareParames.ssdkSetupShareParams(byText: "我正在使用酷课堂，这是一个很棒的ACT&SAT学习平台，你也下载使用吧！",
                                          ...
                                          url : URL(string:"https://itunes.apple.com/cn/app/id1063100471"),
                                          title : "酷课堂 - ACT&SAT",
